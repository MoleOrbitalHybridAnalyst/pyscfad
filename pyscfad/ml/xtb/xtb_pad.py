...
def _scf_q_broyden(
    ...
    def body_fun(value):
        ...
        # get q from dm
        g1 = mf.get_q(mol=mf.mol, dm=dm, s1e=s1e) - q1
        ...
        q2 = normalize_tot_charge(mf, q1 + s1)
#        jax.debug.print("cylce = {} inv_norm = {} q2 = {} u = {} v = {} g1 = {} s1 = {}",
#                        cycle+1,
#                        inv_norm,
#                        np.linalg.norm(q2),
#                        np.linalg.norm(u),
...
#                        )
    ...
    vhf = mf.get_veff(dm=dm, s1e=s1e)
    e_tot = mf.energy_tot(dm=dm, h1e=h1e, vhf=vhf)
    g0 = mf.get_q(mol=mf.mol, dm=dm, s1e=s1e) - q
    q1 = normalize_tot_charge(mf, q + (1 - damp) * g0)
    s0 = q1 - q
